...
public class BzdzData {
    ...
    public static Dataset<Row> Mljbxxb(SparkSession sparkSession){
        String sql="select mldzid,'3' dzjb,dzmc,jlxdm as mljlxdm,jlxmc as dqjlxmc,sjdm,fxjdm,pcsdm,zrqdm,mphhz,mph,mphhz_hfh,mphhz_zhfh,shiid,qxgxid,xzjdbscid,sqdm,sfscfw,xt_zxbz from "+BZDZ_MLJBXXB+"";
        Dataset<Row> Data=sparkSession.sql(sql);
        return Data;
    }

    public static Dataset<Row> Chjbxxb(SparkSession sparkSession){
        String sql="select chdzid,dzmc,parenttreepath,mldzid,dzjb,sjdm,fxjdm,pcsdm,zrqdm,xt_zxbz from "+BZDZ_CHJBXXB+" ";
        Dataset<Row> Data=sparkSession.sql(sql);
        return Data;
    }
    ...
    public static Dataset<Row> Fwjbxxb(SparkSession sparkSession){
        String sql="select id,fwdz_dzid,fwdz_dzxz,gxzrqdm from "+SYFW_FWJBXXB+" ";
        Dataset<Row> Data=sparkSession.sql(sql);
        return Data;
    }
